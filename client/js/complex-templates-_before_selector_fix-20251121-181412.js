...
registerTemplate('selector', {
    ...
    handleSelectorChange: async function(parentId, selectedChildId) {
        ...
        try {
            // Actualitzar BD: marcar només el seleccionat com checked=1
            // Primer, obtenir tots els fills del selector
            const selectorNode = document.querySelector(`[data-form-id="${parentId}"]`);
            if (!selectorNode) return;
            
            const radioInputs = selectorNode.querySelectorAll('input[type="radio"]');
            
            // Desar cada un amb l'estat correcte
            for (const radio of radioInputs) {
                const childId = radio.dataset.childId;
                const shouldBeChecked = childId == selectedChildId;
                
                // Si l'estat actual és diferent del que hauria de ser, actualitzar
